...
export class BootstrapDefaultOrgUseCase {
  ...
  private withMembershipContext(
    session: AuthSession,
    membership: { orgId: string; role: AuthSession['role'] },
  ): AuthSession {
    return {
      ...session,
  ...
  async execute(input: unknown): Promise<Result<AuthSession>> {
    ...
    try {
      ...
      try {
        ...
        // Cloud Functions require Blaze plan; membership fallback supplies org context in the client.
      }
      ...
      try {
        ...
        // Audit is best-effort; org bootstrap should still succeed.
      }

      return ok(refreshed ?? this.withMembershipContext(session, membership));
    } catch (error) {
      return err(error instanceof Error ? error : new Error('Onboarding failed'));
    }
  }
}
