...
export function PosPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Point of Sale"
        description="Create orders, collect payment, and print receipts."
        action={
          ...
          <CardContent>
            <form onSubmit={onSubmit} className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input {...register('items.0.description')} />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input type="number" step="0.01" {...register('items.0.unitPrice')} />
              </div>
              <div className="space-y-2">
                <Label>Tax</Label>
      ...
      <DataTable
        ...
        rows={orders.map((o) => [
          o.id.slice(0, 8),
          `₹${o.total.toFixed(2)}`,
          <Badge key={`${o.id}-s`} variant={o.status === 'paid' ? 'success' : 'outline'}>
            {o.status}
          </Badge>,
          <div key={`${o.id}-a`} className="flex gap-2">
            {o.status === 'draft' && (
              <Button
                size="sm"
                onClick={() => payOrder.mutate({ orderId: o.id, paymentMethod: 'cash' })}
              >
                Mark paid
              </Button>
            )}
            {o.status === 'paid' && (
              <Button size="sm" variant="outline" onClick={() => printReceipt(o.id)}>
                Receipt
              </Button>
            )}
          </div>,
